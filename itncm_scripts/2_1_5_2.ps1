...
function get_network_properties{
    param([Parameter(Mandatory=$true)]$network_config)
    $output_array = @()
    ...
    foreach($o in $output_array){
        write-output("HOST:IP:MAC,$($o.HostName),$($o.IPAddress),$($o.MacAddress)")

    }
}
function get_netstat_processes{  
    param([Parameter(Mandatory=$true)]$netstat_command)  
    $ipv4_array = @()
    $ipv6_array = @()    
...
get_network_properties -network_config $network
write-output("`n### HOST_ADDRESS_INFO_END ###`n")
...
get_netstat_processes -netstat_command $netstat
get_netstat_UDP -netstat_command $netstat
...
$32_bit_apps = get_software -path $32_bit_reg_path
$64_bit_apps = get_software -path $64_bit_reg_path
write_32 -output $32_bit_apps
write_64 -output $64_bit_apps
...
$files = @(
    "G:\repos\powershell\itncm_scripts\test.txt"
)
