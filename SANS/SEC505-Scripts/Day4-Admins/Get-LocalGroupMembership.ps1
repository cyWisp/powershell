...
Param ($ComputerName = "$env:ComputerName", $LocalGroupName = "Administrators", [Switch] $CommaSeparatedOutput)


function Get-LocalGroupMembership ($ComputerName = "$env:ComputerName", $LocalGroupName = "Administrators", [Switch] $CommaSeparatedOutput)
{
    # Construct an object whose properties will hold the output of the function
    $Output = ( $Output = ' ' | select-object ComputerName,LocalGroupName,Members,TimeOfCheck ) 
...
{ 
    Get-LocalGroupMembership -ComputerName $Box -LocalGroupName $LocalGroupName -CommaSeparatedOutput:$CommaSeparatedOutput 
}
